...
import { Listing, PROMOTIONS, Promotion } from "@/types";
...
export default function AdminDashboardClient({ listings }: Props) {
  ...
  const promotionOptions: { value: Promotion | null; label: string }[] = [
    { value: null, label: "Bez promocije" },
    ...PROMOTIONS.map((p) => ({ value: p.value, label: p.label })),
  ];

  async function handleSetPromotion(listingId: string, promotion: Promotion | null) {
    setUpdatingPromoId(listingId);
    try {
          ...
          <tbody className="divide-y divide-gray-50">
            ...
            {filtered.map((listing) => {
              ...
              const imageUrl = firstImage
                ? getPublicImageUrl(firstImage.storage_path)
                : null;
              ...
              return (
                <tr
                  key={listing.id}
                  className={`hover:bg-gray-50/50 transition-colors ${listing.is_hidden ? "opacity-50" : ""}`}
                >
                  ...
                  <td className="px-4 py-3 font-medium text-gray-900">
                    {listing.price.toLocaleString("sr-RS")}{" "}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {promotionOptions.map((opt) => {
                        const isSelected =
                          (opt.value === null && !listing.promotion) ||
                          listing.promotion === opt.value;
                        const isUpdating = updatingPromoId === listing.id;
                        const colorClass =
                          opt.value === null
                            ? "border-gray-200 text-gray-500 hover:border-gray-400"
                            : opt.value === "novo"
                              ? "border-emerald-200 text-emerald-600 hover:border-emerald-400"
                              : opt.value === "poslednja_velicina"
                        ...
                        return (
                          ...
                          >
                            <button
                              type="button"
                              onClick={() =>
                                handleSetPromotion(listing.id, opt.value)
                              }
                              disabled={isUpdating}
                              className={`w-8 h-8 flex items-center justify-center rounded border transition-colors shrink-0 disabled:opacity-50 ${isSelected ? `${colorClass} ring-1 ring-offset-1 ring-current/30` : "border-gray-200 text-gray-300 hover:text-gray-500 hover:border-gray-300"}`}
                            >
                              {isUpdating ? (
                                <svg
                            ...
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2 py-1 rounded bg-gray-900 text-white text-xs font-medium whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-10">
                              {opt.label}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        title={
                          listing.is_hidden ? "Prikaži oglas" : "Sakrij oglas"
                        }
                      >
                        <svg
                          className="w-4 h-4"
      ...
      <div className="md:hidden space-y-3">
        ...
        {filtered.map((listing) => {
          ...
          return (
            <div
              key={listing.id}
              className={`bg-white rounded-xl border border-gray-200 p-4 ${listing.is_hidden ? "opacity-50" : ""}`}
            >
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          disabled={isUpdating}
                          className={`w-8 h-8 flex items-center justify-center rounded border transition-colors shrink-0 disabled:opacity-50 ${isSelected ? `${colorClass} ring-1 ring-offset-1 ring-current/30` : "border-gray-200 text-gray-300 hover:text-gray-500"}`}
                        >
                          {isUpdating ? (
                            <svg
                              className="w-4 h-4 animate-spin"

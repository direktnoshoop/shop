...
export default function StickyContactBar({ listingTitle, price, currency, isSold }: Props) {
  ...
  const primaryHref = hasWhatsApp
    ? buildWhatsAppUrl(listingTitle)
    : hasViber
    ? buildViberUrl(listingTitle)
    : `mailto:${CONTACT.email}?subject=Oglas: ${encodeURIComponent(listingTitle)}`;

  const primaryLabel = hasWhatsApp ? 'WhatsApp' : hasViber ? 'Viber' : 'Email';

  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur border-t border-gray-100 px-4 pb-5 pt-3">
